...
// Mixes four channels, always dividing by 4 (flat scaling, no clamping needed
// in theory -- think about whether that's actually fully true).
int16_t mix_channels_scaled(int16_t ch1, int16_t ch2, int16_t ch3, int16_t ch4) {
    // TODO:
    // 1. Cast each channel up to int32_t before summing (same reasoning as before)
    // 2. Sum all four
    ...
    int32_t sum = (xch1 + xch2 + xch3 + xch4)/4;
    return (int16_t)sum;
}
...
int main(void) {
    ...
    for(uint8_t i = 0; i < 4; i++){
        printf("Sum clamped :%d\n", mix_channels(samples[i][0], samples[i][1], samples[i][2], samples[i][3]));
        printf("Sum scaled :%d\n", mix_channels_scaled(samples[i][0], samples[i][1], samples[i][2], samples[i][3]));
    }
    return 0;
}